...
const inputFields = document.querySelectorAll(".container__input > input");
...
form.addEventListener("submit", (e) => {
  ...
  if (
    !(emailField = document.getElementById("email")).value.match(emailformat)
  ) {
  ...
  fields.forEach((field) => {
    if ((fieldElement = document.getElementById(field)).value.length === 0) {
      ...
      flashMessage.textContent =
        fieldElement.getAttribute("placeholder") + " cannot be empty";
